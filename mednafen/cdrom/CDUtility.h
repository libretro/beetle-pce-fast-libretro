...
extern "C" {
  ...
  *out_number = BCD_to_U8(bcd_number);
  ...
  return(true);
   ...
   static INLINE void TOC_Init(struct TOC *toc)
   {
      TOC_Clear(toc);
   }
 ...
 /* Calculates the checksum of Q subchannel data(not including the checksum bytes of course ;)) 
  * from subq_buf, and stores it into the appropriate position
  * in subq_buf.
  */
