...
/* Audio-track decoder.
 *
 * Used to be a C++ CDAFReader abstract base class with a single
 * concrete CDAFReader_Vorbis subclass.  Now a plain C struct: Vorbis
 * is the only audio backend Beetle Saturn ships (CUE-referenced
 * .wav files that are actually Ogg Vorbis - the Mednafen format
 * convention), so the vtable collapses to direct calls.  All state -
 * the Vorbis decoder + the last-read-position cursor used by the
 * frame_offset != last_read_pos seek shortcut - lives in the struct
