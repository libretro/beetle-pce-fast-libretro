...
public:
   ...
   ~Gf8_Q_Coeffs_Results_01() {}
   const u_int16_t* operator[](int i) const
   {
      return &table[i][0];
   }
   operator const u_int16_t* () const
   {
      return &table[0][0];
   }
...
public:
   CrcTable();
   ~CrcTable() {}
   u_int32_t operator[](int i) const
   {
      return table[i];
   }
   operator const u_int32_t* () const
   {
      return table;
   }
} CRCTABLE;
