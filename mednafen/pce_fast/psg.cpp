...
void PCEFast_PSG::UpdateOutput_Accum(const int32 timestamp, psg_channel* ch)
{
   ...
   samp[0] = ((int32)dbtable_volonly[ch->vl[0]] * ((int32)ch->samp_accum - 496))
             >> (8 + 5);
   samp[1] = ((int32)dbtable_volonly[ch->vl[1]] * ((int32)ch->samp_accum - 496))
             >> (8 + 5);
...
#undef CLOCK_LFSR
   ...
   if (!(ch->control & 0x80) || (chc == 1 && (lfoctrl & 0x80))
         || (ch->control & 0x40))
      return;
