...
static void INLINE Fader_SyncWhich(void)
{
...
static int32 INLINE ADPCM_ClocksToNextEvent(void)
{
...
static uint8 INLINE read_1808(int32 timestamp, const bool PeekMode)
{
...
uint8 MDFN_FASTCALL PCECD_Read(uint32 timestamp, uint32 A, int32 &next_event, const bool PeekMode)
{
	uint8 ret = 0;
...
static void INLINE Fader_Run(const int32 clocks)
{
	if(Fader.Clocked)
	{
...
int32 MDFN_FASTCALL PCECD_Write(uint32 timestamp, uint32 physAddr, uint8 data)
{
	const uint8 V = data;
...
static void INLINE ADPCM_PB_Run(int32 basetime, int32 run_time)
{
...
static int ADPCM_StateAction(StateMem *sm, const unsigned load, const bool data_only)
{
