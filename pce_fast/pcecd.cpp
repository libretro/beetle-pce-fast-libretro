                ...
                Blip_Buffer* soundbuf_r)
{
   ...
   if (!(ADPCM.RAM = (uint8*)MDFN_malloc(0x10000, ("PCE ADPCM RAM"))))
      return (0);

   PCECD_SetSettings(settings);

   ADPCM.bigdivacc = (int64)((double)master_clock * OC_Multiplier * 65536 /
                             32087.5);
...
void PCECD_Write(uint32 timestamp, uint32 physAddr, uint8 data)
{
   ...
   case 0x0:
      ...
      /* reset irq status */
      _Port[0x3] &= ~(0x20 |
                      0x40); // TODO: Confirm writing this register really reset these bits.
      update_irq_state();
      break;
   ...
   case 0xf:
      ...
      {
         Fader.CountValue = OC_Multiplier * 3 * ((V & 0x4) ? 273 :
                                                 655);  // 2.500s : 6.000s;
...
static INLINE void ADPCM_Run(const int32 clocks, const int32 timestamp)
{
   ...
   if (!ADPCM.WritePending)
   {
      if (_Port[0xb] & 0x3)
      {
         ...
         if (!PCECD_Drive_GetCD() && PCECD_Drive_GetIO() && PCECD_Drive_GetREQ()
               && !PCECD_Drive_GetACK())
         {
...
static int ADPCM_StateAction(StateMem* sm, int load)
{
   ...
   int ret = MDFNSS_StateAction(sm, load, StateRegs, "APCM");
   if (load)
   {
...
int PCECD_StateAction(StateMem* sm, int load)
{
   ...
   int ret = MDFNSS_StateAction(sm, load, StateRegs, "PECD");
   ret &= PCECD_Drive_StateAction(sm, load, "CDRM");
   ret &= ADPCM_StateAction(sm, load);
