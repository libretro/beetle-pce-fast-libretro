             ...
             int* erasure_list, int erasure_count)
{
   ...
   if (erasure_count > 0)
   {
      lambda[1] = gt->alphaTo[mod_fieldmax(LEC_PRIM_ELEM * (GF_FIELDMAX - 1 -
                                           erasure_list[0]))];
   ...
   while (++r <= NROOTS)
   {
      ...
      for (i = 0; i < r; i++)
         if ((lambda[i] != 0) && (syndrome[r - i - 1] != GF_ALPHA0))
            discr_r ^= gt->alphaTo[mod_fieldmax(gt->indexOf[lambda[i]] + syndrome[r - i -
                                                1])];
   ...
   for (i = 1, k = LEC_PRIMTH_ROOT - 1; i <= GF_FIELDMAX;
         i++, k = mod_fieldmax(k + LEC_PRIMTH_ROOT))
   {
   ...
   for (j = lambda_roots - 1; j >= 0; j--)
   {
      ...
      {
         corrected++;
         data[location - padding] ^= gt->alphaTo[mod_fieldmax(gt->indexOf[num1] +
                                                 gt->indexOf[num2]
