...
void subpw_deinterleave(const uint8* in_buf, uint8* out_buf)
{
   ...
   for (unsigned ch = 0; ch < 8; ch++)
   {
      for (unsigned i = 0; i < 96; i++)
         out_buf[(ch * 12) + (i >> 3)] |= ((in_buf[i] >> (7 - ch)) & 0x1) << (7 -
                                          (i & 0x7));
   }
...
//
void subpw_synth_leadout_lba(const CDUtility_TOC &toc, const int32 lba,
                             uint8* SubPWBuf)
{
   uint8 buf[0xC];
   uint32 lba_relative;
   ...
   uint8 control = (toc.tracks[toc.last_track].control & 0x4) |
                   toc.tracks[100].control;
...
#if 0
bool subq_extrapolate(const uint8* subq_input, int32 position_delta,
                      uint8* subq_output)
{
