      ...
      std::map<std::string, FSTREAM_ID> &toc_streamcache)
{
   ...
   track->FileOffset =
      offset; // Make sure this is set before calling GetSectorCount()!
   ...
   while (get_line(fp, linebuf) >= 0)
   {
      ...
      if (IsTOC)
      {
         ...
         else if (cmdbuf == "FILE" || cmdbuf == "AUDIOFILE")
         {
            ...
            ParseTOCFileLineInfo(&TmpTrack, active_track, args[0], binoffset, msfoffset,
                                 length, toc_streamcache);
         }
      ...
      else // now for CUE sheet handling
      {
         ...
         else if (cmdbuf == "INDEX")
         {
            if (active_track >= 0)
            {
               ...
               else if (!strcasecmp(args[0].c_str(), "00")
                        || !strcasecmp(args[0].c_str(), "0"))
      ...
      else // else handle CUE sheet...
      {
         ...
         else
         {
            ...
            if (Tracks[x + 1].index[0] == -1)
               ...
               Tracks[x].sectors = Tracks[x + 1].index[0] -
                                   Tracks[x].index[1];   //Tracks[x + 1].index - Tracks[x].index;
         }
...
CDAccess_Image::CDAccess_Image(const char* path) : NumTracks(0), FirstTrack(0),
   LastTrack(0), total_sectors(0)
{
...
void CDAccess_Image::Read_Raw_Sector(uint8* buf, int32 lba)
{
   ...
   for (int32 track = FirstTrack; track < (FirstTrack + NumTracks); track++)
   {
      ...
      if (lba >= (ct->LBA - ct->pregap_dv - ct->pregap)
            && lba < (ct->LBA + ct->sectors + ct->postgap))
      {
         TrackFound = TRUE;
...
void CDAccess_Image::MakeSubPQ(int32 lba, uint8* SubPWBuf)
{
   ...
   if ((lba < Tracks[track].LBA)
         || (lba >= Tracks[track].LBA + Tracks[track].sectors))
   {
   ...
   // Handle pregap between audio->data track
   {
      ...
      if (pg_offset < -150)
      {
         if ((Tracks[track].subq_control & SUBQ_CTRLF_DATA) && (FirstTrack < track)
               && !(Tracks[track - 1].subq_control & SUBQ_CTRLF_DATA))
         {
