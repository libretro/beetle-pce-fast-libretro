...
static void DoREQUESTSENSE(const uint8* cdb)
{
   ...
   MakeSense(data_in, cd.key_pending, cd.asc_pending, cd.ascq_pending,
             cd.fru_pending);
...
static void DoNEC_PCE_SAPSP(const uint8* cdb)
{
   ...
   case 0x40:
      new_read_sec_start = AMSF_to_LBA(BCD_to_U8(cdb[2]), BCD_to_U8(cdb[3]),
                                       BCD_to_U8(cdb[4]));
      break;
...
static void DoNEC_PCE_SAPEP(const uint8* cdb)
{
   ...
   case 0x40:
      new_read_sec_end = BCD_to_U8(cdb[4]) + 75 * (BCD_to_U8(cdb[3]) + 60 * BCD_to_U8(
                            cdb[2]));
      ...
      if (CDReadTimer <= 0)
      {
         ...
         else
         {
            ...
            if (cd.TrayOpen)
            {
               ...
               CommandCCError(SENSEKEY_ILLEGAL_REQUEST, NSE_END_OF_VOLUME);
            else if (!Cur_CDIF->ReadRawSector(tmp_read_buf,
                                              SectorAddr))  //, SectorAddr + SectorCount))
            {
               cd.data_transfer_done = FALSE;
   ...
   if (ResetNeeded)
   {
      ...
      case PHASE_COMMAND:
         ...
         if (!REQ_signal && !ACK_signal
               && cd.command_buffer_pos) // Received at least one byte, what should we do?
         {
...
void PCECD_Drive_Init(int cdda_time_div, Blip_Buffer* leftbuf,
